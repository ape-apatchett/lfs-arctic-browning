...
sm_st_long <- sm_st %>%
  pivot_longer(cols = Temp_1:SM_4, names_to = c(".value", "set"), 
               names_pattern = "(.*)_(.)")
...
sm_st_long <- sm_st_long %>%
  ...
  dplyr::filter(!(Plot %in% c("BE8", "HE8", "BC8", "HC8", "HC9", 
                              "BC9", "BE2a"))) %>%
  droplevels()
...
ggplot(sm_st, aes(Vegetation, SWC, colour = Status, group = Status)) +
  geom_point() +
  geom_smooth(method = "lm") +
  scale_colour_manual(values = c("Browning" = "#996600",
                                 "Healthy" = "#336600")) +
...
table <- sm_st_long %>%
  group_by(Vegetation, Status, Year, Plot, Month, Day) %>%
  summarise(
...
table_formatted <- table %>%
  mutate(
    Status = recode(Status,
                    "Browning" = "Browned"),
    temp_formatted = paste0(
      sprintf("%.2f", temp),
      " \u00B1 ",
      sprintf("%.2f", temp_se)
    ),
    swc_formatted = paste0(
      sprintf("%.2f", swc),
      " \u00B1 ",
      sprintf("%.2f", swc_se)
    )
...
table_output <- kable(
  ...
  format = "html",
  align = "c",
  col.names = c("Vegetation Type", "Health Status", "Year", "n", 
...
((0.416 - 0.329) / 0.329) * 100 #E plots were ~26% wetter than C plots

emmeans(glmsm, ~ Status, type = "response")

((0.346 - 0.397) / 0.397) * 100 #B plots were ~13% drier than H plots
