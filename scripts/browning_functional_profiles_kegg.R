...
#CB and CH P = 0.002, CH and EH P = 0.38
...
##Rank KEGG orthologs by linear model effects####
...
# Fit KO-specific linear models
kegg_stats <- kegg_long %>%
  group_by(KO) %>%
  group_modify(~{
...
# Extract matrix
heat_data_kegg <- t(ko_log[, top_kegg])

# Plot
#40 KOs with the strongest vegetation or health effects
pheatmap(
  heat_data_kegg,
  scale = "row",
  annotation_col = metadata_annot,
...
sig_contrasts <- all_contrasts %>%
  ...
  mutate(
    ...
    Magnitude = if_else(
      ...
      1 / FoldChange,
      FoldChange)) %>%
  relocate(contrast, .after = Feature)

sig_contrasts %>%
  arrange(padj) %>%
  mutate(
    # round p-value columns 
...
ko_category <- KO_map %>%
  mutate(
    priority = case_when(
      grepl("^Protein families", Broad_Cat) ~ 3,
      grepl("^Unclassified", Broad_Cat) ~ 4,
      Broad_Cat == "Poorly characterized" ~ 5,
      TRUE ~ 1
    )
  ) %>%
  arrange(KO_ID, priority) %>%
  distinct(KO_ID, .keep_all = TRUE) %>%
  select(KO_ID, Broad_Cat)

sig_contrasts_annot <- sig_contrasts %>%
  left_join(
    ko_category,
    by = c("Feature" = "KO_ID"))
...
sig_contrasts_annot %>%
  ...
  dplyr::count(Broad_Cat, sort = TRUE)
...
sig_contrasts_annot_noeb <- sig_contrasts_noeb %>%
  left_join(
    KO_map %>%
      distinct(KO_ID, Broad_Cat, Path_cat),
    by = c("Feature" = "KO_ID"))

sig_contrasts_annot_noeb %>%
  filter(!is.na(Broad_Cat)) %>%
  filter(!grepl("^Protein families", Broad_Cat),
         !grepl("^Unclassified", Broad_Cat),
         Broad_Cat != "Poorly characterized") %>%
  dplyr::count(Broad_Cat, sort = TRUE)
...
##Rank nitrogen metabolism KEGG orthologs by linear model effects####
...
nit_long <- nit_log %>%
  ...
  left_join(metadata_nit2, by = "Sample")
...
# Extract matrix
heat_data_nit <- t(nit_log[, top_nit])
...
##Rank methane metabolism KEGG orthologs by linear model effects####
...
met_long <- met_log %>%
  ...
  left_join(metadata_met2, by = "Sample")
...
# Extract matrix
heat_data_met <- t(met_log[, top_met])
