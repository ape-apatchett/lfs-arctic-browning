...
#BE2, BE3, BE4, BE5, BE6, HC1, HC5, HC7, HE1, HE2, HE3, and HE4.
#
#This script:
...
##Rank CAZy families by linear model effects####
...
# Fit family-specific linear models
cazy_stats <- cazy_long %>%
  group_by(Family) %>%
  group_modify(~{
...
# Extract matrix
heat_data_cazy <- t(mat_log[, top_cazy])

# Plot
#40 CAZy families with the strongest vegetation or health effects
pheatmap(
  heat_data_cazy,
  scale = "row",
  annotation_col = metadata_annot,
